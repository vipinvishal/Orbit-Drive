...
from app.jobs.purge_trash import run_purge_trash
...
# Trash has a 30-day retention window (see purge_trash.py) — nothing is ever
# urgent to sweep, so this runs far less often than the account-health jobs.
PERIODIC_PURGE_TRASH_INTERVAL = timedelta(hours=6)
MAX_ATTEMPTS = 10
...
async def run_worker_loop(stop_event: asyncio.Event) -> None:
    ...
    last_purge_trash_run = datetime.min.replace(tzinfo=timezone.utc)
    ...
    while not stop_event.is_set():
        try:
            async with AsyncSessionLocal() as db:
                ...
                if now - last_purge_trash_run >= PERIODIC_PURGE_TRASH_INTERVAL:
                    await run_purge_trash(db)
                    last_purge_trash_run = now
